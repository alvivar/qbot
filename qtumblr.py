"""
    Script that queues Tumblr posts from hardcoded stuff.
"""
...
if __name__ == "__main__":
    ...
    DIR = __file__ if not getattr(sys, 'frozen', False) else sys.executable
    os.chdir(os.path.normpath(os.path.dirname(DIR)))
    ...
    TOKENS = {
        'consumer_key': 'S6ZgUvEvCow4Y02AV6WN26gBZi1CEQ1qBN1dynV93BDnL8grF3',
        'consumer_secret': '6XZrOAVeUxx6u7FN7geJU4sjfEMbMjBew8PoxYUZ5ChHqHDiLJ',
        'oauth_token': 'TuD4tzdJ9oVSE25Qc2Z4SVbckI1uTdqeeAaeMfaLsDxARMsMt4',
        'oauth_secret': '0J6bklbqeMvjV0UBeH2CmYzpebyxH8lBtkBMdXOPWBen86El6W'
    }
    ...
    IMAGES = [i['image'] for i in DATA['messages']]
    ...
    for image_file in IMAGES:
        ...
        year, month, day, *_ = time.gmtime(os.path.getmtime(image_file))
        month = calendar.month_name[month]
        date = re.sub(" +", " ", f"{month}, {year}").strip()  # Umm

        image_count = f"{COUNT} of {DATA_COUNT}"
        text = f"{image_name} ({date})"
        tags = ['gamedev', 'indiedev', 'gif', 'matnesis']
        ...
        try:
            result = API.create_photo(
                ...
                caption=f"{text}",
                data=image_file
                # source=f"{v['gif'] if v['gif'] else v['image']}"  # From web url
            )
        except Exception as e:  # Kind of Unsafe?
            result = False
            print(f"\nError: {e}\n")
            continue
